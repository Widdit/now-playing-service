...
- **网易云音乐进度跳转同步**：通过 UI Automation 实时读取网易云主窗口播放进度，支持拖动进度条后前端歌词即时跟随（详见下方「程序原理」）
...
> **说明**：播放进度通常由 Java 端基于算法递增计算，因此多数平台无法检测拖动进度条动作，但暂停动作可以检测到。该部分使用了 [这段代码](https://stackoverflow.com/questions/23182880/check-if-an-application-emits-sound) 来获取指定进程的实时音量，从而判断音乐是否暂停。
>
> **网易云音乐增强**：针对网易云已单独适配，通过 UI Automation 读取主播放窗口的进度文本（`MM:SS / MM:SS`），可以实时捕捉到拖动进度条的动作，前端歌词会立即跟随跳转。
>
> **备注**：开启此能力后，`GetMusicStatus.exe` 在原先两行输出（状态、窗口标题）之外会额外输出一行 `Progress:currentSec|totalSec`，仅在网易云窗口进度可读取时出现，对其它下游解析属于兼容新增字段；若读取失败（例如迷你模式、托盘模式、或特殊版本 UIA 结构不同）会自动退回原先的两行输出。

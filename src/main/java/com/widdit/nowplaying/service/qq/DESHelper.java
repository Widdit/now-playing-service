...
/**
 * 原作者：WXRIW
 * 代码链接：https://github.com/WXRIW/Lyricify-Lyrics-Helper/blob/master/Lyricify.Lyrics.Helper/Decrypter/Qrc/DESHelper.cs
...
public class DESHelper {
    ...
    private static int BITNUMINTL(int a, int b, int c) {
        ...
        return (int)(((aLong << b) & 0x80000000L) >>> c);
    }
    ...
    private static final byte[] sbox5 = {
    ...
    private static final byte[] sbox6 = {
    ...
    private static final byte[] sbox7 = {
    ...
    private static final byte[] sbox8 = {
    ...
    public static void keySchedule(byte[] key, byte[][] schedule, int mode) {
        int i, j, toGen;
        int C, D;
        int[] key_rnd_shift = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
        int[] key_perm_c = {56, 48, 40, 32, 24, 16, 8, 0, 57, 49, 41, 33, 25, 17,
                9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35};
        int[] key_perm_d = {62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21,
                13, 5, 60, 52, 44, 36, 28, 20, 12, 4, 27, 19, 11, 3};
        int[] key_compression = {13, 16, 10, 23, 0, 4, 2, 27, 14, 5, 20, 9,
        ...
        for (i = 0; i < 16; ++i) {
            C = ((C << key_rnd_shift[i]) | ((C & 0xFFFFFFFF) >>> (28 - key_rnd_shift[i]))) & 0xFFFFFFF0;
            D = ((D << key_rnd_shift[i]) | ((D & 0xFFFFFFFF) >>> (28 - key_rnd_shift[i]))) & 0xFFFFFFF0;
            ...
            for (j = 0; j < 24; ++j)
                schedule[toGen][j / 8] |= (byte) BITNUMINTR(C, key_compression[j], 7 - (j % 8));

            for (; j < 48; ++j)
                schedule[toGen][j / 8] |= (byte) BITNUMINTR(D, key_compression[j] - 27, 7 - (j % 8));
        }
    }
    ...
    private static int F(int state, byte[] key) {
        ...
        state = ((sbox1[SBOXBIT((lrgstate[0] & 0xFF) >>> 2)] << 28) |
                (sbox2[SBOXBIT(((lrgstate[0] & 0x03) << 4) | ((lrgstate[1] & 0xFF) >>> 4))] << 24) |
                (sbox3[SBOXBIT(((lrgstate[1] & 0x0F) << 2) | ((lrgstate[2] & 0xFF) >>> 6))] << 20) |
                (sbox4[SBOXBIT(lrgstate[2] & 0x3F)] << 16) |
                (sbox5[SBOXBIT((lrgstate[3] & 0xFF) >>> 2)] << 12) |
                (sbox6[SBOXBIT(((lrgstate[3] & 0x03) << 4) | ((lrgstate[4] & 0xFF) >>> 4))] << 8) |
                (sbox7[SBOXBIT(((lrgstate[4] & 0x0F) << 2) | ((lrgstate[5] & 0xFF) >>> 6))] << 4) |
    ...
    public static void tripleDESKeySetup(byte[] key, byte[][][] schedule, int mode) {
        if (mode == ENCRYPT) {
            keySchedule(key, schedule[0], mode);
            keySchedule(Arrays.copyOfRange(key, 8, key.length), schedule[1], DECRYPT);
            keySchedule(Arrays.copyOfRange(key, 16, key.length), schedule[2], mode);
        } else {
            keySchedule(key, schedule[2], mode);
            keySchedule(Arrays.copyOfRange(key, 8, key.length), schedule[1], ENCRYPT);
            keySchedule(Arrays.copyOfRange(key, 16, key.length), schedule[0], mode);
        }
    }

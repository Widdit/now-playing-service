...
public class KuGouMusicSMTC : MusicService
{
    ...
    public override string GetMusicStatus(AudioSessionManager2 sessionManager)
    {
        ...
        {
            // 获取播放状态
            // 对于酷狗音乐，如果不打开 Windows 的媒体控制面板，则 SMTC 中的播放状态可能不会刷新，因此还是需要通过检测音量来判断播放/暂停
    ...
    {
        if (session.Id.Contains("kugou") || session.Id.Contains("KuGou") || session.Id.Contains("酷狗"))
        {
            hasSession = true;

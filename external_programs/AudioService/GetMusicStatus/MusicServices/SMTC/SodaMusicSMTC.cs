...
public class SodaMusicSMTC : MusicService
{
    ...
    public override string GetMusicStatus(AudioSessionManager2 sessionManager)
    {
        ...
        {
            // 获取播放状态
            // 对于汽水音乐，如果不打开 Windows 的媒体控制面板，则 SMTC 中的播放状态可能不会刷新，因此还是需要通过检测音量来判断播放/暂停
    ...
    {
        if (session.Id.Contains("SodaMusic") || session.Id.Contains("汽水") || session.Id.Contains("soda"))
        {
            hasSession = true;
    ...
    private double GetVolume(AudioSessionManager2 sessionManager)
    {
        ...
        foreach (AudioSessionControl session in sessionEnumerator)
        {
            ...
            if (processName.StartsWith("SodaMusic"))
            {

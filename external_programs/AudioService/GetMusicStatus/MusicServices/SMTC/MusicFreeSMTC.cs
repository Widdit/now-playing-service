...
public class MusicFreeSMTC : MusicService
{
    ...
    public override string GetMusicStatus(AudioSessionManager2 sessionManager)
    {
        ...
        {
            // 获取播放状态
            // 对于 MusicFree，如果歌曲处于暂停状态，SMTC 中的播放状态却是播放中，因此还是需要通过检测音量来判断播放/暂停
    ...
    {
        if (session.Id.Contains("MusicFree") || session.Id.Contains("musicfree"))
        {
            hasSession = true;
